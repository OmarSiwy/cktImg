...
fn from_spice(src: &str, it: &mut Interner) -> Schematic<Unplaced> {
    ...
    if src.split_whitespace().any(|t| t.eq_ignore_ascii_case("vdd")) {
        text.push_str("XVDD vdd vdd\n");
    }
    if src.split_whitespace().any(|t| t.eq_ignore_ascii_case("gnd")) {
...
pub fn diode_connected(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/diode_connected.spice"), it)
}

pub fn common_source(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/common_source.spice"), it)
}

pub fn cs_isource_load(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/cs_isource_load.spice"), it)
}
...
pub fn source_degenerated(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/source_degenerated.spice"), it)
}

// ---- B. two spline / cross-coupled ----

pub fn current_mirror(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/current_mirror.spice"), it)
}

pub fn cascode_current_mirror(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/cascode_current_mirror.spice"), it)
}

pub fn wilson_mirror(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/wilson_mirror.spice"), it)
}

pub fn cross_coupled_pair(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/cross_coupled_pair.spice"), it)
}

// ---- C. shared / branching ----

pub fn differential_pair(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/differential_pair.spice"), it)
}

pub fn tail_current_source(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/tail_current_source.spice"), it)
}
...
pub fn push_pull(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/push_pull.spice"), it)
}

// ---- D. multi-stage / feedback ----

pub fn two_stage_miller(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/two_stage_miller.spice"), it)
}

pub fn gain_boosted_cascode(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/gain_boosted_cascode.spice"), it)
}

pub fn three_stage_nested_miller(it: &mut Interner) -> Schematic<Unplaced> {
    from_spice(include_str!("../../../../tests/fixtures/three_stage_nested_miller.spice"), it)
}
