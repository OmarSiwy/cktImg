    ...
    for (name, f) in circuits::all() {
        ...
        for col in &cols {
            ...
            let xs: Vec<i32> = col
                .devices
                .iter()
                .map(|&d| ev.physical.pos[d.index()].x)
                .collect();
            assert!(
                xs.iter().all(|&x| x == xs[0]),
                "{name}: column not on one axis: {xs:?}"
            );
            let ys: Vec<i32> = col
                .devices
                .iter()
...
fn classify_partitions_by_column_span() {
    use ColumnKind::*;
    let all_sp = [
        Spline, Spline, Spline, Spline, Spline, Spline, Spline, Spline,
    ];
    ...
    assert_eq!(
        classify(&[1, 2, 3], &all_sp),
        Case::SpanGe2,
        "consecutive but span 2 → staple"
    );
    ...
    assert_eq!(
        classify(&[0, 2], &with_shared),
        Case::ImmediateNeighbor,
        "Shared column is transparent"
    );
}

/// Break C: a rail-less circuit (transmission gate) yields NO splines — devices sharing both
/// conducting nets are grouped into one signal-series column (antiparallel). No rail feeds the
/// group, so devices lie FLAT with opposing gates (up/down) and side wires join the terminals.
#[test]
fn splineless_circuit_is_signal_series() {
    ...
    assert!(
        cols.iter().all(|c| c.kind == ColumnKind::SignalSeries),
        "all columns are signal-series"
    );
    ...
    assert!(
        grouped >= 1,
        "antiparallel devices should be grouped into one column"
    );

    let ev = evaluate(&ctx, &[]);
    // A pass group has no rail feeding it: devices lie flat with opposing gates.
    ...
    assert!(
        non_rail
            .iter()
            .all(|&d| matches!(ev.orient[d].rot(), Rot::R0 | Rot::R180)),
        "antiparallel pass devices should lie flat"
    );
    // Gates point in opposite directions
    let rots: Vec<Rot> = non_rail.iter().map(|&d| ev.orient[d].rot()).collect();
    assert!(
        rots.len() >= 2 && rots[0] != rots[1],
        "antiparallel gates must point opposite directions"
    );
}
...
fn cs_isource_load_and_gain_boosted_place_as_specified() {
    ...
    assert_eq!(
        splines.len(),
        1,
        "CS + current-source load is a single spline"
    );
    assert_eq!(
        splines[0].len(),
        2,
        "two stacked devices share the output node"
    );
    ...
    assert!(
        ...
        "gain-boosted cascode places deterministically"
    );
}
