...
fn rect_intersects_is_strict() {
    ...
    assert!(
        a.intersects(&Rect::new(Pt::new(5, 5), Pt::new(15, 15))),
        "overlapping interiors collide"
    );
    assert!(
        !a.intersects(&Rect::new(Pt::new(10, 0), Pt::new(20, 10))),
        "edge-to-edge does not collide"
    );
    assert!(
        !a.intersects(&Rect::new(Pt::new(10, 10), Pt::new(20, 20))),
        "corner touch does not collide"
    );
    assert!(
        !a.intersects(&Rect::new(Pt::new(20, 20), Pt::new(30, 30))),
        "disjoint"
    );
    ...
    assert!(
        wire(-5, 15, 5).intersects(&a),
        "wire through the interior collides"
    );
    assert!(
        !wire(-5, 15, 0).intersects(&a),
        "wire flush along the bottom edge is clear"
    );
    assert!(
        !wire(-5, 15, 10).intersects(&a),
        "wire flush along the top edge is clear"
    );
...
fn direct_horizontals_never_cross_a_device_box() {
    ...
    for (name, f) in circuits::all() {
        ...
        let boxes: Vec<(i32, Rect)> = (0..ctx.nd())
            .map(|d| {
                (
                    phys.pos[d].x,
                    dev_box(&ev.orient, &ctx, DeviceIdx(d as u32), phys.pos[d]),
                )
            })
            .collect();
        ...
        for n in 0..ctx.nn() {
            ...
            for &(cx, b) in &boxes {
                if x0 < cx && cx < x1 {
                    assert!(
                        !wire.intersects(&b),
                        "{name} net{n}: direct run {p:?}->{q:?} crosses device box {b:?}"
                    );
                    checked += 1;
                }
            }
    ...
    assert!(
        checked >= 1,
        "expected at least one direct horizontal with an intervening column to vet"
    );
}
...
fn body_hits_metric_is_real_and_minimised() {
    for (name, f) in circuits::all() {
        ...
        for n in 0..ctx.nn() {
            ...
            let net_devs: Vec<DeviceIdx> =
                ctx.members(net).iter().map(|&p| ctx.dev_of(p)).collect();
        ...
        assert_eq!(
            measured, ev.metrics.num_body_hits,
            "{name}: body-hit metric disagrees with drawn geometry"
        );
        assert!(
            best <= ev.metrics.num_body_hits,
            "{name}: a better order exists ({best} < {})",
            ev.metrics.num_body_hits
        );
    }
}
